...
def backup_given_date(backup_type,src_dir,remote_dir,start_date_str,end_date_str):
    """
	...
	for date_dir in ret:
	    ...
	    if file_date <= end_date and file_date >= start_date:
            	ret1 = os.system("hadoop dfs -cat {src}/* 1>{backup_root}/{date} 2>/dev/null".format(src=date_dir,backup_root=backup_type,date=file_date_str))
		    ...
		    #print "hadoop dfs -cat {src}/* 1>{backup_root}/{date} 2>/dev/null".format(src=date_dir,backup_root=backup_type,date=file_date_str)
            	    ret2 = os.system("cd {backup_root} && tar -czvf {date}.tar.gz_{backup_date} {date} 2>/dev/null".format(backup_root=backup_type, date=file_date_str, backup_date=today))
                    if ret2 != 0:
	...
	print "total processed {0}".format(len(success_list) + len(failed_list))
        ...
        print "{0} backup failed, please ensure that {0} contains %Y-%m-%d like subdir".format(d)
...
if __name__ == "__main__":
    #test_containDate()
    backup_given_date("xtx_tracking","/user/hive/external/tw_tr_datas_xtx_d","root@10.0.0.172:/home/ubuntu/backup/hdfs/test/","2015-08-01","2015-09-01")
